...
class MiniBatchKMeansSegmenter(DefineParams):
    def __init__(self,file_name, image, n_clusters, batch_size=100):
        self.image = image
    ...
    def segment_algo(self):
        ...
        minibatch_kmeans = MiniBatchKMeans(n_clusters=self.n_clusters, batch_size=self.batch_size)
        labels = minibatch_kmeans.fit_predict(pixel_values)
        return labels
    ...
    def color_image(self, image):
        img = self.file_name[0] + "_miniBkmeans." + self.file_name[1]
