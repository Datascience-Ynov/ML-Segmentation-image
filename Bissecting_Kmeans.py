...
class BissectingKMeansSegmenter(DefineParams):
    ...
    def segment_algo(self):
        ...
        bisecting_kmeans_model = BisectingKMeans(n_clusters=self.n_clusters)
        labels = bisecting_kmeans_model.fit_predict(pixel_values)
    ...
    def create_segmented_image(self, labels, color_palette):
        ...
        segmented_image = cv2.resize(segmented_image, (self.original_shape[1], self.original_shape[0]), interpolation=cv2.INTER_NEAREST)

        return segmented_image
    
    def color_image(self, image):
        img = self.file_name[0] + "_bissecting_kmeans." + self.file_name[1]
