...
class K_meansSegmenter(DefineParams):
    def __init__(self, file_name,image, n_clusters=5):
        self.image = image
    ...
    def segment_algo(self):
        ...
        centroids, labels, inertia = k_means(pixel_values, n_clusters=self.clusters)
        labels = labels.reshape(self.resized_image.shape[:2])
        return labels
    ...
    def color_image(self, image):
        img = self.file_name[0] + "_k_means." + self.file_name[1]
