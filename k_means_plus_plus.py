...
class KMeansPlusPlusSegmenter(DefineParams):
    def __init__(self, file_name,image, clusters):
        self.image = image
    ...
    def segment_algo(self):
        ...
        centroids, _ = kmeans_plusplus(pixel_values, n_clusters=self.clusters, random_state=42)

        kmeans = KMeans(n_clusters=self.clusters, init=centroids, n_init=1)
    ...
    def color_image(self, image):
        img = self.file_name[0] + "_k_means_plus." + self.file_name[1]
